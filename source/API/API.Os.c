...
void API_UpdateScene(API_EntList *cl) {
    ...
    if (Cursor) {
        if ((keysDown() & KEY_DOWN) && Cursor->down) {
            Cursor = Cursor->down;
        } else if ((keysDown() & KEY_RIGHT) && Cursor->right) {
            Cursor = Cursor->right;
        } else if ((keysDown() & KEY_LEFT) && Cursor->left) {
            Cursor = Cursor->left;
        } else if ((keysDown() & KEY_UP) && Cursor->up) {
            Cursor = Cursor->up;
        }
    ...
    do {
        ...
        if (!e2->bastard && e2->type == 1) {
            ...
            if ((e->father == e2->father || force == 2) && (e2->alpha > 0 || force == 1) && pos2.x < inttof32(256) && pos2.x > 0 && pos2.y>-inttof32(192) && pos2.y < inttof32(192)) {
                ...
                if (pos2.x > pos.x && dr > d) {
                    dr = d;
                    e->right = e2;
                }
                if (pos2.x < pos.x && dl > d) {
                    dl = d;
                    e->left = e2;
                }
                if (pos2.y > pos.y && dd > d) {
                    dd = d;
                    e->down = e2;
                }
                if (pos2.y < pos.y && du > d) {
                    du = d;
                    e->up = e2;
                }
            }
