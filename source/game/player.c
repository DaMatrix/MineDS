...
#else
    char path[255];
    sprintf(path, "%s/%s/settings.ini", basePath, ROOT);
...
void updatePlayer(player_struct* p) {
    ...
    testLine[1].x += lineOfSight.x * 32;
    testLine[1].y += lineOfSight.y * 32;
    testLine[1].z += lineOfSight.z * 32;
...
vect3D Normal(vect3D* vTriangle) { // Get 2 vectors from the polygon (2 sides), Remember the order!
...
vect3D ClosestPointOnLine(vect3D vA, vect3D vD, int length, int32* dist, vect3D vPoint) {
    ...
    vect3D vVector2 = vD; //vD direction du segment, norm�
