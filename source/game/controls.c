...
void controlScheme1(void) {
    ...
    if (!noclip) {
        ...
        if ((keys & KEY_LEFT) || (keys & KEY_Y)) {
            walkSfx++;
            Player.vector.y += cosLerp(Player.angleZ - 8192) >> 3;
            Player.vector.x += sinLerp(Player.angleZ - 8192) >> 3;
            if (!Player.vector.z)walkAngle += 2500;
        }
        if ((keys & KEY_RIGHT) || (keys & KEY_A)) {
            walkSfx++;
            Player.vector.y += cosLerp(Player.angleZ + 8192) >> 3;
            Player.vector.x += sinLerp(Player.angleZ + 8192) >> 3;
            if (!Player.vector.z)walkAngle += 2500;
    ...
    else if ((keysDown() & KEY_TOUCH) && doubletap && !noclip && (Player.inWater || !Player.vector.z) && !invOpen && !overButtons) {
        Player.vector.z += 850;
        doubletap = 0;
    }
...
void controlScheme2(void) {
    ...
    if (!noclip && (Player.inWater || !Player.vector.z) && (keysDown() & KEY_A)) {
        Player.vector.z += 850;
    }
    if (keysUp() & KEY_R) {
        cubeAngleX = -1;
...
void updateControls(void) {
    if (testBuffer) {
        ...
        if (!noclip && !Player.inWater && !Player.onLadder) {
            // if((keysDown() & KEY_A)) {Player.vector.z += 1400;}
            if ((keys & KEY_UP) || (!gameSettings.controls && (keys & KEY_X))) {
                walkSfx++;
                Player.vector.y = cosLerp(Player.angleZ) >> 3;
                Player.vector.x = sinLerp(Player.angleZ) >> 3;
                if (!Player.vector.z)walkAngle += 2500;
            }
            if ((keys & KEY_DOWN) || (!gameSettings.controls && (keys & KEY_B))) {
                walkSfx++;
                Player.vector.y = -cosLerp(Player.angleZ) >> 3;
                Player.vector.x = -sinLerp(Player.angleZ) >> 3;
                if (!Player.vector.z)walkAngle -= 2500;
            ...
            if ((keys & KEY_UP) || (!gameSettings.controls && (keys & KEY_X))) {
                Player.vector.y = mulf32(cosLerp(Player.angleZ), cosLerp(Player.angleX)) >> 3;
                Player.vector.x = mulf32(sinLerp(Player.angleZ), cosLerp(Player.angleX)) >> 3;
                Player.vector.z += -sinLerp(Player.angleX) >> 6;
            }
            if ((keys & KEY_DOWN) || (!gameSettings.controls && (keys & KEY_B))) {
                Player.vector.y = -mulf32(cosLerp(Player.angleZ), cosLerp(Player.angleX)) >> 3;
                Player.vector.x = -mulf32(sinLerp(Player.angleZ), cosLerp(Player.angleX)) >> 3;
            ...
            if ((keys & KEY_UP) || (!gameSettings.controls && (keys & KEY_X))) {
                Player.vector.y = mulf32(cosLerp(Player.angleZ), cosLerp(Player.angleX)) >> 2;
                Player.vector.x = mulf32(sinLerp(Player.angleZ), cosLerp(Player.angleX)) >> 2;
                Player.vector.z = 1024;
            }
            if ((keys & KEY_DOWN) || (!gameSettings.controls && (keys & KEY_B))) {
                Player.vector.y = -mulf32(cosLerp(Player.angleZ), cosLerp(Player.angleX)) >> 2;
                Player.vector.x = -mulf32(sinLerp(Player.angleZ), cosLerp(Player.angleX)) >> 2;
            ...
            if ((keys & KEY_UP) || (!gameSettings.controls && (keys & KEY_X))) {
                Player.vector.y = mulf32(cosLerp(Player.angleZ), cosLerp(Player.angleX)) >> 3;
                Player.vector.x = mulf32(sinLerp(Player.angleZ), cosLerp(Player.angleX)) >> 3;
                Player.vector.z = -sinLerp(Player.angleX) >> 3;
            }
            if ((keys & KEY_DOWN) || (!gameSettings.controls && (keys & KEY_B))) {
                Player.vector.y = -mulf32(cosLerp(Player.angleZ), cosLerp(Player.angleX)) >> 3;
                Player.vector.x = -mulf32(sinLerp(Player.angleZ), cosLerp(Player.angleX)) >> 3;
        ...
        if (oldwAngle != walkSfx && ((walkSfx) >= 10) && !Player.vector.z) {
            mmEffect(SFX_STEP);
            walkSfx = 0;
        }
        oldwAngle = walkSfx;
