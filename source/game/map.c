...
void makeIcon(u16* buff, int id, int i1, int j1, int i2, int j2) {
    ...
    for (x = 0; x < 16; x++) {
        for (y = 0; y < 16; y++) {
            ...
            if (y < 4) {
                if (abs(8 - x) < y * 2) {
                    *dest = getPixel(i1, j1, dx, dy2, buff, 32);
                } else *dest = RGB15(0, 0, 0);
            } else if (y < 4 * 2 && abs(8 - x) < 16 - y * 2) {
                *dest = getPixel(i1, j1, dx, dy2, buff, 32);
...
static inline void addListElement(list_struct* l, u16 i, u16 j, u16 k, u8 direction) {
    l->elements[l->size].i = i;
    l->elements[l->size].j = j;
    l->elements[l->size].k = k;
    l->elements[l->size].direction = direction;
    l->size++;
}
...
void cullClusters2(map_struct* m, list_struct* ol, list_struct* cl, int sI, int sJ, int sK) {
    ...
    if (!testBuffer) {
        ...
        bid = qgetCluster(m, sI, sJ, sK);
        m->clusterDraw[bid] = cullMagic;
        if (sI < SUPERCLUSTERSIZE - 1) {
            addListElement(ol, sI + 1, sJ, sK, dir_x);
            m->clusterDraw[bid + 1] = cullMagic;
        }
        if (sI > 0) {
            addListElement(ol, sI - 1, sJ, sK, dir_x);
            m->clusterDraw[bid - 1] = cullMagic;
        }
        if (sJ < SUPERCLUSTERSIZE - 1) {
            addListElement(ol, sI, sJ + 1, sK, dir_y);
            m->clusterDraw[bid + cx] = cullMagic;
        }
        if (sJ > 0) {
            addListElement(ol, sI, sJ - 1, sK, dir_y);
            m->clusterDraw[bid - cx] = cullMagic;
        }
        if (sK < m->clusterSize.z - 1) {
            addListElement(ol, sI, sJ, sK + 1, dir_z);
            m->clusterDraw[bid + cxy] = cullMagic;
        }
        if (sK > 0) {
            addListElement(ol, sI, sJ, sK - 1, dir_z);
            m->clusterDraw[bid - cxy] = cullMagic;
    ...
    for (i = olcursor; i + 1 < ol->size && n < 400 && count < 1700; i++) //test
    {
        ...
        if (r || i < 6) //test
        {
            ...
            if (!(d3 && le->direction & dir_x)) {
                if (le->i < SUPERCLUSTERSIZE - 1 && m->clusterDraw[bid + 1] != cullMagic) {
                    addListElement(ol, le->i + 1, le->j, le->k, newdir);
                    m->clusterDraw[bid + 1] = cullMagic;
                }
                if (le->i > 0 && m->clusterDraw[bid - 1] != cullMagic) {
                    addListElement(ol, le->i - 1, le->j, le->k, newdir);
                    m->clusterDraw[bid - 1] = cullMagic;
                }
            }
            if (!(d2 && le->direction & dir_y)) {
                ...
                if (le->j < SUPERCLUSTERSIZE - 1 && m->clusterDraw[bid + cx] != cullMagic) {
                    addListElement(ol, le->i, le->j + 1, le->k, newdir);
                    m->clusterDraw[bid + cx] = cullMagic;
                }
                if (le->j > 0 && m->clusterDraw[bid - cx] != cullMagic) {
                    addListElement(ol, le->i, le->j - 1, le->k, newdir);
                    m->clusterDraw[bid - cx] = cullMagic;
                }
            }
            if (!(d1 && le->direction & dir_z)) {
                ...
                if (le->k < m->clusterSize.z - 1 && m->clusterDraw[bid + cxy] != cullMagic) {
                    addListElement(ol, le->i, le->j, le->k + 1, newdir);
                    m->clusterDraw[bid + cxy] = cullMagic;
                }
                if (le->k > 0 && m->clusterDraw[bid - cxy] != cullMagic) {
                    addListElement(ol, le->i, le->j, le->k - 1, newdir);
                    m->clusterDraw[bid - cxy] = cullMagic;
                }
            }
...
void cullClusters(map_struct* m, list_struct* ol, list_struct* cl, int sI, int sJ, int sK) {
    ...
    for (i = olcursor; i + 1 < ol->size && n < 400 && count < 1700; i++) //test
    {
        ...
        if (r || i < 6) //test
        {
            ...
            if (r < 3) {
                m->clusterDrawn[bid] = cullMagic + o % 3;
                o++;
            }
            n++;
            m->clusterDraw[bid] = cullMagic;
        ...
        for (j = y; j < y + CLUSTERSIZE; j++) {
            for (k = 0; k < 16 * CLUSTERSIZE; k++) {
                if (!*getBlockPE(m, i, j, k))t[(i - x)+(j - y) * CLUSTERSIZE + k * CLUSTERSIZE * CLUSTERSIZE] = 0;
                    ...
                    t[(i - x)+(j - y) * CLUSTERSIZE + k * CLUSTERSIZE * CLUSTERSIZE] = (((transparent3(m, i, j, k, i, j, k - 1))&1))
    ...
    for (k = 0; k < CLUSTERSIZE * 16; k++) {
        if (!(k % CLUSTERSIZE)) {
            ...
            }
            {
                ...
                c->specialList.first = NULL;
                c->specialList.count = 0;
            }
            {
        ...
        for (j = 0; j < CLUSTERSIZE; j++) {
            ...
            for (i = 0; i < CLUSTERSIZE; i++) {
                ...
                if ((*t)&63) {
                    ...
                    if (*d >= WATERTYPE) {
                        ql = &c->specialList;
                        if (((*t)&63) != 2 || *d > WATERTYPE)addWater(m, x + i, y + j, k, *d); //� optimisay //pas s�r que la nouvelle condition corrige tout
                    } else ql = &c->quadList;
                    if (*d >= LADDERTYPE && *d < LADDERTYPE + 4 + 16) {
                        ...
                        const u8 light = ((((*t) >> 6)&1) << 7);
                        if ((*t)&1) {
                            addQuad(ql, m, 1, light, bid, cC->data, x + i, y + j, k);
                        }
                        if (((*t) >> 1)&1) {
                            addQuad(ql, m, 0, light, bid, cC->data, x + i, y + j, k);
                        }
                        if (((*t) >> 2)&1) {
                            addQuad(ql, m, 5, light, bid, cC->data, x + i, y + j, k);
                        }
                        if (((*t) >> 3)&1) {
                            addQuad(ql, m, 4, light, bid, cC->data, x + i, y + j, k);
                        }
                        if (((*t) >> 4)&1) {
                            addQuad(ql, m, 3, light, bid, cC->data, x + i, y + j, k);
                        }
                        if (((*t) >> 5)&1) {
                            addQuad(ql, m, 2, light, bid, cC->data, x + i, y + j, k);
                        }
#ifdef FOGLIGHT
...
void translateSuperCluster(map_struct* m, u8 dir) {
    ...
    switch (dir) {
        case 0:
            for (j = 0; j < 32; j++) {
                if (m->superCluster[30][j]->changed) {
                    globalSaveMap(m);
                    break;
                }
            }
        ...
        case 2:
            for (j = 0; j < 32; j++) {
                if (m->superCluster[j][30]->changed) {
                    globalSaveMap(m);
                    break;
                }
            }
...
void initMap(map_struct* m, vect3D clusterSize) {
    ...
    m->size.x = clusterSize.x*CLUSTERSIZE;
    m->size.y = clusterSize.y*CLUSTERSIZE;
    m->size.z = clusterSize.z*CLUSTERSIZE;
    ...
    initSuperCluster(m);
    for (i = 0; i < SUPERCLUSTERSIZE * SUPERCLUSTERSIZE * m->clusterSize.z; i++) {
        m->clusterDraw[i] = 0;
    }
    for (i = 0; i < SUPERCLUSTERSIZE * SUPERCLUSTERSIZE * m->clusterSize.z; i++) {
        m->clusterDrawn[i] = 0;
            ...
            for (i2 = 0; i2 < 8; i2++) {
                for (j2 = 0; j2 < 8; j2++) {
                    ...
                    for (k = 0; k < m->size.z; k++) {
                        ...
                        else if (tree && k - v < tree + 2) {
                            (*getBlockP(m, i, j, k)) = 10;
...
void readClusterColumn2048(map_struct* m, u16 i, u16 j, clusterColumn_struct* c, u8* t, void* f) {
    PROF_START();
    //porktodo: readSectors(m->fileMap[i+m->clusterSize.x*j], 2, c->data);
    //porktodo: readSectors(m->fileMap[i+m->clusterSize.x*j]+2, 2, t);
    int time;
    PROF_END(time);
...
void readClusterColumn1024(map_struct* m, u16 i, u16 j, clusterColumn_struct* c, u8* t, void* f) {
    PROF_START();
    //porktodo: readSectors(m->fileMap[i*2+m->clusterSize.x*2*j], 2, c->data);
    //porktodo: readSectors(m->fileMap[i*2+1+m->clusterSize.x*2*j], 2, t);
    int time;
    PROF_END(time);
    addValue(&streamRead, time);
    PROF_START();
...
void readClusterColumn512(map_struct* m, u16 i, u16 j, clusterColumn_struct* c, u8* t, void* f) {
    PROF_START();
    //porktodo: readSectors(m->fileMap[i*4+m->clusterSize.x*4*j], 1, c->data);
    //porktodo: readSectors(m->fileMap[i*4+1+m->clusterSize.x*4*j], 1, &c->data[512]);
    //porktodo: readSectors(m->fileMap[i*4+2+m->clusterSize.x*4*j], 1, t);
    //porktodo: readSectors(m->fileMap[i*4+3+m->clusterSize.x*4*j], 1, &t[512]);
    int time;
    PROF_END(time);
    addValue(&streamRead, time);
    PROF_START();
...
void writeClusterColumn2048(map_struct* m, u16 i, u16 j, clusterColumn_struct* c, u8* t, void* f) {
    ...
    //porktodo: writeSectors(m->fileMap[i+m->clusterSize.x*j], 2, c->data);
    //porktodo: writeSectors(m->fileMap[i+m->clusterSize.x*j]+2, 2, t);
    PROF_END(time);
...
void writeClusterColumn1024(map_struct* m, u16 i, u16 j, clusterColumn_struct* c, u8* t, void* f) {
    ...
    PROF_START();
    precalcCollumn(m, i, j, t);
    //porktodo: writeSectors(m->fileMap[i*2+m->clusterSize.x*2*j], 2, c->data);
    //porktodo: writeSectors(m->fileMap[i*2+1+m->clusterSize.x*2*j], 2, t);
    PROF_END(time);
    addValue(&columnWrite, time);
    if (!c->changed)c->changed = 2;
}
...
void writeClusterColumn512(map_struct* m, u16 i, u16 j, clusterColumn_struct* c, u8* t, void* f) {
    ...
    PROF_START();
    precalcCollumn(m, i, j, t);
    //porktodo: writeSectors(m->fileMap[i*4+m->clusterSize.x*4*j], 1, c->data);
    //porktodo: writeSectors(m->fileMap[i*4+1+m->clusterSize.x*4*j], 1, &c->data[512]);
    //porktodo: writeSectors(m->fileMap[i*4+2+m->clusterSize.x*4*j], 1, t);
    //porktodo: writeSectors(m->fileMap[i*4+3+m->clusterSize.x*4*j], 1, &t[512]);
    PROF_END(time);
    addValue(&columnWrite, time);
    if (!c->changed)c->changed = 2;
}
...
void writeMapHeader(map_struct* m) {
    ...
    switch (fsFormat) {
        case 1:
            //porktodo: writeSectors(m->headerMap[0], 4, (u8*)m->header);
            break;
        case 2:
            //porktodo: writeSectors(m->headerMap[0], 2, &(((u8*)m->header)[0]));
            //porktodo: writeSectors(m->headerMap[1], 2, &(((u8*)m->header)[1024]));
            break;
        case 3:
            //porktodo: writeSectors(m->headerMap[0], 1, &(((u8*)m->header)[0]));
            //porktodo: writeSectors(m->headerMap[1], 1, &(((u8*)m->header)[512]));
            //porktodo: writeSectors(m->headerMap[2], 1, &(((u8*)m->header)[1024]));
            //porktodo: writeSectors(m->headerMap[3], 1, &(((u8*)m->header)[1024+512]));
            break;
        case 0:
