...
bool updateInterface(void) {
    ...
    if (overButtons) {
        if ((keysUp() & KEY_TOUCH) && lastXY.px >= TOGGLEX && lastXY.py >= TOGGLEY && lastXY.px < TOGGLEX + TOGGLESX && lastXY.py < TOGGLEY + TOGGLESY) {
            ...
            oamSub.oamMemory[buttonSprite + 2 * 0].attribute[0] = ATTR0_DISABLED;
            oamSub.oamMemory[buttonSprite + 2 * 0 + 1].attribute[0] = ATTR0_DISABLED;
        }//else VRAM_I_EXT_SPR_PALETTE[0][TOGGLECOL]=RGB15(31,31,31);
        ...
        if ((keysHeld() & KEY_TOUCH) && thisXY.px >= INVX && thisXY.py >= INVY && thisXY.px < INVX + INVSX && thisXY.py < INVY + INVSY) {
            ...
            oamSub.oamMemory[buttonSprite + 2 * 1].attribute[0] = ATTR0_DISABLED;
            oamSub.oamMemory[buttonSprite + 2 * 1 + 1].attribute[0] = ATTR0_DISABLED;
        }//else VRAM_I_EXT_SPR_PALETTE[0][INVCOL]=RGB15(31,31,31);
        ...
        if ((keysHeld() & KEY_TOUCH) && thisXY.px >= SAVEX && thisXY.py >= SAVEY && thisXY.px < SAVEX + SAVESX && thisXY.py < SAVEY + SAVESY) {
            ...
            oamSub.oamMemory[buttonSprite + 2 * 2].attribute[0] = ATTR0_DISABLED;
            oamSub.oamMemory[buttonSprite + 2 * 2 + 1].attribute[0] = ATTR0_DISABLED;
        }
    }
    // vramSetBankI(VRAM_I_SUB_SPRITE_EXT_PALETTE);
    return !invOpen;
